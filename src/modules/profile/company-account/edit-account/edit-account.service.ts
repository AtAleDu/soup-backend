...
export class EditCompanyAccountService {
  ...
  async updateProfile(userId: string, dto: UpdateCompanyAccountDto) {
    ...
    if (dto.profile) {
      ...
      if (dto.profile.logo !== undefined)
        updateData.logo_url = dto.profile.logo;
    ...
    if (dto.contacts) {
      if (dto.contacts.phones !== undefined) {
        ...
        const phones = dto.contacts.phones
          .filter(
            (item): item is { phone: string; representativeName?: string } =>
      ...
      if (dto.contacts.email !== undefined) {
        updateData.email =
          typeof dto.contacts.email === "string" &&
    ...
    if (hasProfileUpdate && !isLogoOnlyUpdate) {
      const maxRegions = await this.getMaxRegionsByTariff(userId);
      this.validateRequiredProfileFields(company, updateData, maxRegions);
    }
  ...
  /** Возвращает лимит регионов по тарифу: число или null = без ограничений. */
  private async getMaxRegionsByTariff(userId: string): Promise<number | null> {
    const user = await this.users.findOne({
      where: { id: userId },
      relations: ["tariff"],
    });
    const features = user?.tariff?.features as
      | { regions?: number | "all" }
      | undefined;
    if (!features?.regions) return null;
    if (features.regions === "all") return null;
    return typeof features.regions === "number" ? features.regions : null;
  }
  ...
  private validateRequiredProfileFields(
    ...
    const phones = (updateData.phones ?? company.phones ?? []) as Array<{
      phone?: string | null;
    }> | null;
    const email = (updateData.email ?? company.email) as
      | string
      | null
      | undefined;
    ...
    if (
      ...
      throw new BadRequestException(
        `По вашему тарифу можно указать не более ${maxRegions} регионов`,
      );
    }
  ...
  async uploadCompanyLogo(userId: string, file) {
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          "Размер логотипа должен быть от 16 до 512 пикселей",
        );
      }
